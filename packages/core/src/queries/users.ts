...
// Note: Avatar utilities (compressAvatar, generateAvatarPath) are platform-specific
// and should be imported from @tripthreads/web or @tripthreads/mobile
 ...
 */
// TODO: Move uploadAvatar to @tripthreads/web - it requires browser-specific APIs
// Platform-specific implementations should be created in @tripthreads/web and @tripthreads/mobile
export async function uploadAvatar(
  supabase: SupabaseClient<Database>,
  file: File | Blob,
  userId: string,
  compressAvatarFn: (file: File | Blob) => Promise<Blob>,
  generateAvatarPathFn: (userId: string) => string
): Promise<string | null> {
  try {
    // Compress avatar using platform-specific function
    const compressedBlob = await compressAvatarFn(file as File)

    // Generate storage path using platform-specific function
    const storagePath = generateAvatarPathFn(userId)
...
export async function completeProfile(
  ...
  // TODO: Avatar upload should be handled by platform-specific code (web or mobile)
  // Upload avatar if provided
  let avatarUrl: string | undefined

  if (data.avatar) {
    // TODO: Pass platform-specific compress and path generation functions
    throw new Error(
      'Avatar upload must be implemented in platform-specific code (@tripthreads/web or @tripthreads/mobile)'
    )
    // const storagePath = await uploadAvatar(supabase, data.avatar, authUser.id, compressAvatarFn, generateAvatarPathFn)
    // if (storagePath) {
