...
export const CATEGORY_KEYWORDS: Record<string, string[]> = {
  food: ['dinner', 'lunch', 'breakfast', 'meal', 'restaurant', 'cafe', 'coffee', 'drinks', 'groceries', 'food'],
 ...
 * - 2500
 */
export function extractAmount(input: string, decimalFormat: 'US' | 'EU' = 'US'): {
  amount: number;
  position: number;
} | null {
  // Patterns for different number formats (ordered by specificity)
  const patterns = decimalFormat === 'US'
    ? [
...
export function extractNames(input: string): string[] {
  ...
  if (withMatch) {
    ...
    const extracted = namesStr
      ...
      .filter((n) => {
        ...
        return n.length >= 2 && n.length <= 20 && /^[a-z]+$/i.test(n) && !noiseWords.includes(normalized);
      })
  ...
  if (capitalizedWords) {
    ...
    const nonNames = [
      'I', 'Me', 'My', 'The', 'A', 'An', 'This', 'That', 'Split', 'Paid', 'Owes',
      // Common expense descriptions/categories
      'Dinner', 'Lunch', 'Breakfast', 'Meal', 'Restaurant', 'Cafe', 'Coffee', 'Drinks', 'Groceries', 'Food',
      'Taxi', 'Uber', 'Lyft', 'Bus', 'Train', 'Flight', 'Gas', 'Petrol', 'Parking',
      'Hotel', 'Hostel', 'Airbnb', 'Rent', 'Accommodation', 'Room',
      'Tickets', 'Museum', 'Tour', 'Attraction', 'Cinema', 'Movie', 'Concert', 'Show',
    ];
 ...
 */
export function inferCategory(description: string): { category: string; confidence: number } | null {
...
export function extractDescription(input: string): string {
  ...
  description = description.replace(/\b(?:split|divide|share|equally|evenly|between|paid|owes?|owe)\b/gi, '');
